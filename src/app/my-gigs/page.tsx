...
export default async function MyGigs() {
  ...
  return <div className='space-y-3'><h1 className='text-2xl font-bold'>My gigs</h1>{gigs.map(g=><div key={g.id} className='rounded border border-zinc-700 p-3'>
    ...
    <form action={updateGigAction} className='mt-2 flex gap-2'><input type='hidden' name='gig_id' value={g.id}/><button name='status' value='cancelled' className='rounded border px-2 py-1'>Cancel</button><button name='status' value='approved' className='rounded border px-2 py-1'>Re-activate</button></form>
  </div>)}</div>;
}
